...
class AudioPreprocessor:
    ...
    def analyze_quality(self, audio: np.ndarray, sample_rate: int = 16000) -> AudioQualityMetrics:
        ...
        if silence_ratio > self.MAX_SILENCE_RATIO:
            warnings.append(f"High silence ratio ({silence_ratio:.1%}) - recording may be too quiet")
    ...
    def _apply_noise_gate(self, audio: np.ndarray, threshold: float = 0.005, ratio: float = 0.5) -> np.ndarray:
        """
    ...
    def analyze_and_log(self, audio: np.ndarray, sample_rate: int = 16000, label: str = "audio") -> AudioQualityMetrics:
        """
